...
/* test('Check elasticsearch sql', async () => {
  ...
  var index = 1;
  //printLog(' fetch '+index,result.data);
  printLog(' count ', index, await result.count());
  var current = 0;
  /*   while(result.next){
      index++;
...
/* test('Check image upload from url', async () => {
  ...
  await uploadObject.uploadFromUrl(
    ...
    , function (snapshot: any) {
      ...
      //  printLog('222222 Upload is ' + progress + '% done');
      switch (snapshot.state) {
        case firebase.storage.TaskState.PAUSED: // or 'paused'
          //  printLog('222222 Upload is paused');
          break;
        case firebase.storage.TaskState.RUNNING: // or 'running'
          //     printLog('222222 Upload is running');
          break;
      }
    }, function (error: any) {
  ...
  //    printLog('product ===== ',product.getData());
  await product.save();
  //printLog('task ===== ',task);
